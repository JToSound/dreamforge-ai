...
These components are combined to produce discrete sleep stages (Wake, N1, N2, N3, REM) that follow typical overnight structure (N3 dominating early night, lengthening REM episodes towards morning).
...
- REM more likely when circadian drive is high and the night has progressed.
...
- **Cortisol** — follows a circadian-like curve peaking before wake and modulating emotional tone.
...
- Cortisol levels rise toward the end of the night and can be perturbed by stress inputs.

Pharmacological modulation (e.g., SSRIs) is modeled as parameter multipliers that alter effective 5-HT levels, with direct effects on mood and potentially REM dynamics.
...
- **Recency** — time since encoding.

Edges encode associative strengths, emotional alignment, and contextual overlap. Hippocampal sharp-wave ripple (SWR) events are approximated as biased random walks over this graph, favoring nodes with high salience and activation and optionally constrained by tags.

Selective forgetting is implemented as salience and activation decay with emotion-dependent protection: highly emotional memories decay more slowly, reflecting their persistence.
...
- **Implausibility** — internally inconsistent or impossible scenarios.
...
Lucidity is treated as a continuous probability rather than a binary switch. When probability crosses a threshold, the dream state can be flagged as lucid, and dream dynamics (e.g., control, stability) can be altered.
...
- Incorporating more detailed circadian models and environmental zeitgebers.
- Extending the memory graph with real-world knowledge bases and lifelogging data.
- Validating bizarreness metrics against large dream report corpora.
- Modeling pharmacology more accurately (dose, half-life, receptor-level kinetics).
