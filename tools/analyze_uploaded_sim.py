...
def analyze(sim):
    ...
    neuro_stats = {stage: {k: mean_std(v) for k, v in measures.items()} for stage, measures in vals_by_stage.items()}
    ...
    if cortisol_times:
        ...
        second_mean = sum(cort_vals[max(1,half):]) / max(1, len(cort_vals)-max(1,half))
        cortisol_trend = {"first_half_mean": first_mean, "second_half_mean": second_mean, "delta": second_mean - first_mean}
    else:
        cortisol_trend = {"first_half_mean": None, "second_half_mean": None, "delta": None}
