...
def run_validation():
    ...
    lucid_times = [
        s.get("time_hours")
        for s in segments
        if float(s.get("lucidity_probability", 0.0)) > 0.35
    ]
    if len(lucid_times) == 0:
        ...
        print(
            f"Mean lucid time (hours): {avg_lucid_time:.3f} over {len(lucid_times)} segments"
        )
    ...
    if avg_lucid_time is not None:
        assert (
            avg_lucid_time > 4.0
        ), f"Lucid moments not clustered late-night: {avg_lucid_time}"
