...
def plot_biz_hist(segments, outdir, sim_id):
    vals = [s.get('bizarreness_score') or s.get('bizarreness') for s in segments if (s.get('bizarreness_score') or s.get('bizarreness')) is not None]
    if not vals:
        return None
    plt.figure(figsize=(8, 4))
...
def plot_cortisol_curve(neuro_series, outdir, sim_id):
    ...
    if half > 0:
        ...
        plt.axhline(mean1, color='gray', linestyle='--', label=f'first-half mean={mean1:.3f}')
        plt.axhline(mean2, color='red', linestyle='--', label=f'second-half mean={mean2:.3f}')
    plt.xlabel('Time (hours)')
    plt.legend()
...
def plot_memory_activation_heatmap(memory_series, outdir, sim_id, top_n=50):
    ...
    mat = []
    for frame in memory_series:
        amap = {a.get('id'): float(a.get('activation', 0.0)) for a in frame.get('activations', [])}
...
def main(path):
    ...
    if mem_activation:
        results['mem_activation_heatmap'] = plot_memory_activation_heatmap(mem_activation, outdir, sim_id)
