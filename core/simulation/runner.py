...
class SimulationRunner:
    ...
    def __init__(self, sim: Dict[str, Any]):
        ...
        self.neuro = self.sim.get("neurochemistry") or self.sim.get("neurochemistry_series") or []
        self.mem_events = []
        mg = self.sim.get("memory_graph") or {}
        # Replay events may be exported under different keys
        self.mem_events = mg.get("replay_events") or mg.get("replay_event_log") or self.sim.get("replay_event_log") or []
    ...
    def run(self) -> Iterator[SimulationTick]:
        ...
        if self.neuro and isinstance(self.neuro, list) and len(self.neuro) > 0:
            ...
            for i, n in enumerate(timeline):
                ...
                seg = self.segments[seg_idx] if seg_idx is not None and seg_idx < len(self.segments) else {}
                # gather replay events near this time (within small epsilon)
                events = [e for e in self.mem_events if abs(float(e.get("time_hours", 0.0)) - t) < 1e-3]
                tick = SimulationTick(
                    time_hours=t,
                    segment_index=seg_idx or 0,
                    stage=(seg.get("stage") or "N2"),
                    ach=float(n.get("ach", 0.0)),
                    serotonin=float(n.get("serotonin", n.get("5ht", 0.0) if n.get("5ht") is not None else 0.0)),
                    ne=float(n.get("ne", 0.0)),
                    cortisol=float(n.get("cortisol", 0.0)),
                    biz=float(seg.get("bizarreness") or seg.get("bizarreness_score") or 0.0),
                    active_memory_ids=seg.get("active_memory_ids") or [],
                    narrative=seg.get("narrative") or seg.get("scene_description") or "",
            ...
            for i, s in enumerate(self.segments):
                ...
                tick = SimulationTick(
                    ...
                    serotonin=float((s.get("neurochemistry") or {}).get("serotonin", 0.0)),
                    ne=float((s.get("neurochemistry") or {}).get("ne", 0.0)),
                    cortisol=float((s.get("neurochemistry") or {}).get("cortisol", 0.0)),
                    biz=float(s.get("bizarreness") or s.get("bizarreness_score") or 0.0),
